...
- **Style:** Abstract, conceptual, or illustrative — not generic stock photo clichés (handshakes, lightbulbs, stock office scenes). Claude Design generation is the default choice for heroes when connected; otherwise the hand-built SVG pipeline (see below) covers geometric/diagram heroes. A stock photo may be used if it's treated per the Stock Images rules below so it doesn't read as generic
...
- **Approved sources only:** use libraries that grant free commercial use with no attribution requirement (e.g., Unsplash, Pexels, Pixabay for stills; GIPHY or a similarly licensed source for pre-made animated GIFs). Confirm the specific image's license before using it, not just the general site policy.
...
- **When to reach for stock instead of a diagram:** the hand-built diagram pipeline below is geometric (nodes, lines, cards). It can't show human expression or photographic realism. When an article needs that, a personal moment, a reaction, something a diagram can't convey, use stock photography instead of forcing a diagram to do a photo's job.

### Hand-Built Diagrams and Animation

When Claude Design isn't connected in the working session, original diagrams are still required, not skipped. This is a proven, working method (used for C2's article images and the README's animated pipeline diagram), not a fallback of last resort.

- **Static diagrams:** an original SVG matching the palette and type scale in this document, rasterized to PNG (via `sharp` or a headless browser screenshot, e.g. Playwright). See `scripts/render-article-images.mjs` for the working pattern.
- **Animated GIFs:** a sequence of SVG frames generated by a script (not hand-drawn frame by frame), each rasterized to PNG via `sharp`, then assembled into a GIF with `ffmpeg` using a two-pass palette for quality. See `public/assets/readme/generate-pipeline-frames.cjs` for the working pattern.
- **Use animation sparingly:** only where motion clarifies something static can't, a sequence, a state change, a pipeline in motion. Not every diagram needs to move.
- **File size:** keep it light. The README's 9-stage animation is ~115KB; treat that as the target range.
- **Accessibility:** GIFs autoplay and loop with no built-in pause control, so keep animations short and subtle rather than attention-grabbing. Always write alt text describing what the animation shows, motion itself isn't perceivable to screen readers.
- **Keep the source:** the frame-generation script is a source file, kept alongside the article (or in `public/assets/readme/` for site-wide assets) for provenance and future edits, same as a static source SVG.
- **Credit:** `imageCredit: "AI-generated with Claude"`, same as Claude Design output.
