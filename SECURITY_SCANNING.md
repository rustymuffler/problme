...
| 2026-07-22 | rl-protect-skills | `fast-xml-parser@5.10.1` (fix for CVE-2026-59869-adjacent Trivy HIGH finding on the previously-installed `5.9.3`, transitive dep of `@astrojs/rss@4.0.19`) GOVERNANCE FAIL — version published 6 days ago (recency gate); transitive deps `xml-naming@0.3.0` and `is-unsafe@2.0.0` also FAIL the same recency gate | GOVERNANCE | All six substantive checks passed on every package (no malware, no tampering, no CVEs, no secrets, no license issues, no hardening issues). The FAIL is the recency gate only, same pattern as `yargs`, `@playwright/test`, and `tar` above. The two transitive deps flagged (`xml-naming`, `is-unsafe`) are already present under the currently-installed `5.9.3` with identical governance flags, so this override introduces no new dependency, only a version bump of ones already in the tree. Applied alongside clean, non-blocked bumps of `sharp@0.35.0` and `svgo@4.0.2` (also Trivy HIGH fixes) in the same change. Richard explicitly acknowledged and approved. | Production dependency — `fast-xml-parser` ships as part of `@astrojs/rss`'s feed generation. Risk is negligible: governance gate flags recency only, not a real finding, and the flagged transitive deps were already in the tree pre-bump. Recency gate will clear automatically. |
| 2026-08-17 | rl-protect-skills | `puppeteer-core@25.7.0` GOVERNANCE FAIL — version published 3 days ago (recency gate); `@puppeteer/browsers@3.2.0` GOVERNANCE FAIL — version published 5 days ago (recency gate); fix for GHSA-jmr9-qjv8-65gv / CVE-2026-56876 (HIGH, extract-zip symlink path traversal, GitHub Dependabot alert #92) | GOVERNANCE | `extract-zip` is a transitive dep of `@lhci/cli` → `lighthouse@12.6.1` → `puppeteer-core` → `@puppeteer/browsers`, has no patched release (last commit 2021, package effectively abandoned), and is only reachable at all if `@puppeteer/browsers` needs to download and extract its own Chromium build (`chrome-launcher` finds system Chrome first in this project's actual usage, so the vulnerable code path was likely never hit in practice — but the vulnerable package was still present in the tree). Fix: `overrides` forces `puppeteer-core` to `25.7.0`, which pulls `@puppeteer/browsers@3.2.0` — a version that dropped `extract-zip` entirely for a different extraction library, `modern-tar@0.8.4` (scanned clean, full APPROVE). Both new packages pass every substantive check (no malware, no tampering beyond the note below, no CVEs, no secrets, no license issues, no hardening issues); the FAILs are the recency gate only, same pattern as `yargs`/`@playwright/test`/`tar`/`fast-xml-parser` above. Both also roll up a WARN for "1 component with malware history," which traces to `debug@4.4.3` — already the resolved top-level `debug` version in this project before this fix (not newly introduced), and the identical finding already logged and accepted in this table on 2026-06-25 for the same package/version. The one real compatibility risk (`lighthouse@12.6.1` declares `puppeteer-core: ^24.10.0`, so this override goes outside its declared range) was verified empirically, not assumed: a full local Lighthouse audit ran end-to-end successfully via `lhci collect`, and the actual `@puppeteer/browsers`-driven Chrome download/extraction path (the literal code that replaced `extract-zip`) was exercised directly and confirmed working — a real Chrome build was downloaded and extracted correctly. An independent second review (per the Infra-only PR rule in `AGENTS.md`) re-ran all of the above from scratch and confirmed every claim. Richard explicitly acknowledged and approved. | Dev-only tooling (`npm run test:lhci`, `.github/workflows/lighthouse.yml`) — never shipped to the static site output. `npm audit` reports 0 vulnerabilities after the fix; `extract-zip` confirmed fully removed from the dependency tree. Recency gates on `puppeteer-core`/`@puppeteer/browsers` will clear automatically. |
