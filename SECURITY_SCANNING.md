...
| 2026-07-22 | rl-protect-skills | `fast-xml-parser@5.10.1` (fix for a Trivy HIGH finding on the previously-installed `5.9.3`, transitive dep of `@astrojs/rss@4.0.19`) GOVERNANCE FAIL — version published 6 days ago (recency gate); transitive deps `xml-naming@0.3.0` and `is-unsafe@2.0.0` also FAIL the same recency gate | GOVERNANCE | All six substantive checks passed on every package (no malware, no tampering, no CVEs, no secrets, no license issues, no hardening issues). The FAIL is the recency gate only, same pattern as `yargs`, `@playwright/test`, and `tar` above. The two transitive deps flagged (`xml-naming`, `is-unsafe`) are already present under the currently-installed `5.9.3` with identical governance flags, so this bump introduces no new dependency, only a version bump of ones already in the tree. Bundled with two related, non-blocked Trivy HIGH fixes discovered in the same scan: `svgo@4.0.2` and `sharp@0.35.0`, both rl-protect clean. All three had to land in one PR: `main`'s branch protection requires the Trivy check to pass, and no single-package fix clears the check while the other two vulnerable versions remain in the lockfile. (Dependabot opened separate single-package PRs for `fast-xml-parser` (#77) and `svgo` (#78) before this was understood; both were superseded by this consolidated fix.) Richard explicitly acknowledged and approved the fast-xml-parser override. | Production dependency — `fast-xml-parser` ships as part of `@astrojs/rss`'s feed generation. Risk is negligible: governance gate flags recency only, not a real finding, and the flagged transitive deps were already in the tree pre-bump. Recency gate will clear automatically. |
